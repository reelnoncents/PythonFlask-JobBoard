...
def employer(employer_id):
    ...
    jobs = execute_sql('SELECT job.id, job.ititle, job.description. job.salary FROM job join employer ON employer.id = job.employer_id WHERE employer.id = ?', [employer_id])
    ...
    return render_template('employer.html', employer=employer, job=jobs, reviews=reviews)
